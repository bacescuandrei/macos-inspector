...
1. Download `macos-inspector-1.4.1-macos.zip` and `SHA256SUMS` from the latest GitHub release.
...
7. Record the investigation state as `Expected`, `Suspicious`, `Contained`, or `Resolved`. Notes remain local and do not modify scan evidence.
8. Run the same profile again after containment or remediation and compare it with the earlier scan.
