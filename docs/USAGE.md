...
Standalone HTML reports include a visible scope note when a finding is `Not Applicable`. A category with no assessed findings displays `N/A` instead of a category score. When the entire scan has no assessed findings, the overall index displays `N/A` in the history, standalone HTML, Markdown, and PDF reports. Score changes in comparisons are also `N/A` when either scan has no assessed findings; finding-level changes remain available. JSON retains the raw numeric value and adds assessed-finding counts. SARIF uses a null score when no finding was assessed.
An all-`Not Applicable` category is excluded from the overall index instead of contributing a misleading 100. An `Unknown` category remains in the raw calculation because the intended check lacked evidence. In exported category tables, status availability includes completed `Not Applicable` results and is not a measure of target assessment.
