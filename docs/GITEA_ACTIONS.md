...
If the Gitea instance is configured with `DEFAULT_ACTIONS_URL=self` instead of its normal GitHub-compatible action source, mirror `actions/checkout`, `actions/setup-python`, and `christopherhx/gitea-upload-artifact` into the Gitea instance or update the workflow to their fully qualified mirror URLs.

The workflow uses Gitea's v4 artifact backend through the patched `christopherhx/gitea-upload-artifact@v4` action. This backend is supported by Gitea 1.22 and later; the repository server was confirmed to be Gitea 1.27.1 when the workflow was introduced.
